  ...
  switch (id) {
    ...
    case 0x294:
      if (len >= 4) {
        ...
        // BYTE[1] = DIMMER: 0x00=dim  0x01..0x15=smooth(mid)  0x96=MAX
...
void updateMonitorRawValue() {
  ...
  tft.fillRect(10, 78, 300, 22, C_BG);
  tft.setTextColor(C_DIVIDER, C_BG);
  sprintf(tmp, "IDX: %s", idx);
  tft.drawString(tmp, 10, 78);
}
...
void drawDimmer() {
  ...
  if (fillW < 0) fillW = 0;
  uint16_t barColor = (dimmerValue >= 0x96) ? C_RED : C_ACCENT;
  ...
  tft.setTextSize(1);
  tft.setTextColor(C_GREY, C_BG);
  sprintf(tmp, "0x%02X  dim=0  mid=0x15  MAX=0x96", dimmerValue);
...
void updateDimmerValue() {
  ...
  // Update bar
  int barW = 300, barH = 30, barX = 10, barY = 120;
  tft.fillRect(barX, barY, barW, barH, C_LIST_BG);
  tft.drawRect(barX, barY, barW, barH, C_DIVIDER);
  int fillW = map(dimmerValue, 0, 150, 0, barW);
  if (fillW > barW) fillW = barW;
...
void updateSpeedoValue() {
  // Only redraw if the relevant value actually changed
  static int lastDrawnSpeed = -2;
  static int lastDrawnRpm = -2;
  char tmp[50];

  if (speedoMode == SPEEDO_MODE_SPEED) {
    if (speedoValue == lastDrawnSpeed) return;
    lastDrawnSpeed = speedoValue;
    ...
    bool spdChanged = (speedoValue != lastDrawnSpeed);
    bool rpmChanged = (rpmValue != lastDrawnRpm);
    if (!spdChanged && !rpmChanged) return;
...
void processCanData() {
  ...
  while (fifoHead != fifoTail) {
    ...
    if (capture) {
      bool monChanged = !prevMonitorHasData;
      if (monitorLen != msg.len) monChanged = true;
      else if (memcmp(monitorBytes, msg.data, msg.len) != 0) monChanged = true;
      monitorHasData = true;
      monitorLen = msg.len;
