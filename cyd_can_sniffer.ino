 ...
 * v1.2 — Новая компоновка MONITOR экрана
 *
...
int brightnessVal = 0;    // 0-100%
int displayRawVal = 0;    // raw 0-255 (для показа числа)
...
// prototypes
void drawScreen(); void updateMonitorValue(); void handleTouch(); bool canInit();
    ...
    for (int i = 0; i < itemsOnPage; i++) {
      if (hit(x, y, 6, ry, 308, 30)) {
        ...
        if (idx < sortedCount) {
          ...
          brightnessVal = 0; displayRawVal = 0;
